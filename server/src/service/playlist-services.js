...
export const playlistGenerator = async (req) => {
  ...
  const genresDuration = req.body.genres.map((g) =>
    g.duration === 0
      ? g
      : { ...g, duration: Math.round((duration * g.duration) / 100) }
  );
  const genresName = Object.values(
    req.body.genres
      .map((g) =>
        g.duration === 0 ? { ...g, duration: null } : { ...g, duration: g.name }
      )
      .reduce((acc, g) => {
        const a = { [g.name]: g.duration };
        return { ...acc, ...a };
      }, {})
  );
    ...
    const result = gen.map((g) => {
      ...
      while (totalDuration < g.duration) {
        ...
        tracksFiltered = tracksFiltered.filter(
          (t) => t.duration < g.duration * 1.05 - totalDuration
        );
      }
      return temp;
    });
  ...
  const tracksList = generateTracksList(tracksAll, genresDuration).reduce(
    (acc, arr) => [...acc, ...arr],
    []
  );
  const playlistDuration = tracksList.reduce((acc, t) => acc + t.duration, 0);
  const averagePlaylistRank = Math.round(
    tracksList.reduce((acc, t) => acc + t.rank, 0) / tracksList.length
  );
  ...
  await Promise.all(
    tracksList.map(async (track) =>
      playlistData.addTrackToPlaylist(newPlaylist.id, track.id, track.deezer_id)
    )
  );
  await Promise.all(
    req.body.genres.map(async (g) => {
      if (g.duration !== 0) {
